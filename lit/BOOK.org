...
Pandoc converts that org to LaTeX, running the three filters in
order: mermaid.lua renders diagrams, notes.lua handles captions,
references, and asides, plan.lua styles the plan. texlinks.py then
...
Three Lua filters run inside pandoc, in the order the weave recipe
lists them.
